...
import tech.scedu.workbench.enums.CheckEnum;
...
import static tech.scedu.workbench.enums.CheckEnum.JSON_TYPE;
import static tech.scedu.workbench.enums.CheckEnum.MAP_TYPE;
...
public class ExcelUtils {
	 ...
	 */
	public static void exportExcel(CheckEnum check, Map<String, String> map, List<?> list, String fileName) {
		//只导出前一万条数据
		if (list.size() > 10000) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			for (int j = 0; j < map.size(); j++) {
				...
				Object value = getter(list.get(i), attrMap.get(Integer.toString(j)), check);
	 ...
	 * @param attrName 属性名
	 */
	private static Object getter(Object obj, String attrName, CheckEnum check) {
		if (JSON_TYPE.equals(check)) {
			// 改成 JSONObject
			JSONObject jsonObject = (JSONObject) obj;
			return jsonObject.get(attrName);
		} else if (MAP_TYPE.equals(check)) {
	...
	public static void downloadExcel(Map<String, String> map, String excelTemplateName, String sign) {
		...
		while (it.hasNext()) {
			...
			cell = row.createCell(index);
			cell.setCellValue(key);
			if (isNotBlank(sign) && sign.contains(key)) {
				//必填的单元格标记为红色字体
				cell.setCellStyle(style3);
			} else {
				cell.setCellStyle(style2);
	...
	public static List<Map<Object, Object>> getExcelResolverFieldList(List<Map<String, Object>> matchFieldList, File file, Integer beginLine, List<Object> fields) {
		//获取未设置匹配的列表
		List<String> disMatchList = new ArrayList<>(fields.size());
		//已经匹配的列表
		List<String> matchList = new ArrayList<>(matchFieldList.size());
		...
		return DevExcelUtils.resolver(file, fields, disMatchList, beginLine);
	}
